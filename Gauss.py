...
a = numpy.array([[4, 2, -1, -1], [-3, 1, 1, -1], [-1, 4, 5, -8]])
print(a)
...
def gaussFunc(a):  # В метод передавайте вещественный массив))
    ...
    for g in range(len1):
        ...
        if abs(max) < eps:  # Если найденный максимум < 0(машинного) выбрасываем ошибку
            raise DetermExeption(
                "Check determinant"
            )  # тк определитель матрицы равен нулю
...
def vectorN(
    c, a, len1
):  # c-начальная матрица a-ответ len1-кол столбцов, vectB-вектор B
    c = numpy.array(c)
    a = numpy.array(a)
